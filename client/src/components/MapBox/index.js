...
class MapBox extends Component {
    constructor( props ) {
        super( props );
        this.state = {
            pinsArray: this.props.pinsArray
        };
    }
    ...
    componentDidMount() {
        ...
        firebase.database().ref( "/pins" ).orderByChild( "groupID" ).equalTo( this.props.gGroupName ).on( "value", snapshot => {
            ...
            console.log( snapshot.val());
            const newPinsArray = this.state.pinsArray;
            newPinsArray.push( snapshot.val());
        ...
        navigator.geolocation.getCurrentPosition( function( position ) {
            ...
            if ( "geolocation" in navigator ) {
                ...
                positionRef = event => {
                    ...
                    firebase.database().ref( "/pins" ).push({ groupID: this.props.gGroupName,
                                                              name: this.props.gName, x: latitude, y: longitude });
                }
        ...
        return (
            <div>
                <Map google = { this.props.google }
                    ...
                    zoom = { this.props.gZoom }
                    name = { this.props.gName }>
                  {/* <Marker onClick = { this.props.gOnMarkerClick } title = {'Thompson Conference Center'} name = { this.props.gname }/>
                  <InfoWindow onClose = { this.props.gOnClose }></InfoWindow> */}
                </Map>
            </div>
        );
    }
